...
class Student_model extends MY_Model
{
    ...
    public function getSingleStudent($id = '')
    {
        ...
        $this->db->select('s.*, e.class_id, e.section_id, e.id as enrollid, e.roll, e.branch_id, e.session_id, 
                          c.name as class_name, se.name as section_name, sc.name as category_name, l.active');
        
        $this->db->from('student s');
        $this->db->join('enroll e', 'e.student_id = s.id', 'left');
        $this->db->join('login_credential l', 'l.user_id = s.id AND l.role = 7', 'inner');
        $this->db->join('class c', 'e.class_id = c.id', 'left');
        $this->db->join('section se', 'e.section_id = se.id', 'left');
        $this->db->join('student_category sc', 's.category_id = sc.id', 'left');
        
        // Add index hints if needed (uncomment if your database supports it)
        // $this->db->query('USE INDEX (PRIMARY, student_id, class_id, section_id)');
        ...
        // Add a limit to prevent multiple rows (just in case)
        $this->db->limit(1);
        ...
        // Log if we got multiple rows (shouldn't happen with limit 1, but just in case)
        if ($query->num_rows() > 1) {
            log_message('error', 'Multiple students found with the same ID: ' . $id);
        }
