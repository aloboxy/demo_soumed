...
class Student extends Admin_Controller
{
    ...
    protected function student_validation()
    {
        ...
        $this->form_validation->set_rules('email', translate('email'), 'trim|required|callback_unique_username');
    ...
    public function generate_idcard()
    {
        ...
        if ($_POST) {
            ...
            $branch = $this->branch_model->getBranch($this->data['branch_id']);
            // Add QR code image for each student
            if ($this->data['query']->num_rows() > 0) {
                $students = $this->data['query']->result();
                foreach ($students as $row) {
                    $qrData = "School Name: " . $branch->school_name . 
                              " | Reg_ID: " . $row->register_no;
                    $row->qr_code = $this->create_qrcode($qrData);
                }
    ...
    public function create_qrcode($id = null)
    {
        ...
        $save_path = FCPATH . 'uploads/qrcode_cache/';
        if (!is_dir($save_path)) {
        ...
        // Generate only if not already saved
