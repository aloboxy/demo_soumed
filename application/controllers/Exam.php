...
class Exam extends Admin_Controller
{
    ...
    public function marksheet()
    {
        ...
        if ($_POST) {
            ...
            // Group by all non-aggregated columns to be compatible with ONLY_FULL_GROUP_BY mode
            $this->db->group_by(array(
                'm.student_id', 'e.roll', 's.id', 's.register_no', 's.admission_date', 
                's.first_name', 's.last_name', 's.gender', 's.birthday', 's.religion', 
                's.caste', 's.blood_group', 's.mother_tongue', 's.current_address', 
                's.permanent_address', 's.city', 's.state', 's.mobileno', 's.category_id', 
                's.email', 's.parent_id', 's.photo', 'c.name', 's.created_at', 's.updated_at'
            ));
